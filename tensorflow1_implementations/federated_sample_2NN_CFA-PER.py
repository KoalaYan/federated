...
logging.basicConfig(filename='2NN_CFA-PER.log', level=logging.INFO)
...
parser.add_argument('-P', default=0.0, help="sets the package error rate", type=float)
...
def processData(samples, iii, federated, tot_devices,fraction_training, neighbors_number, probability):
...
if __name__ == "__main__":
    ...
    for ii in range(devices):
        t.append(multiprocessing.Process(target=processData, args=(int(samples[ii]), ii, federated, devices, validation_train, neighbors_number, probability)))
        pause(1)
        t[ii].start()
    exit(0)
